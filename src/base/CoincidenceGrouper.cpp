	...
	: systemConfig(systemConfig), UnorderedEventHandler<GammaPhoton, Coincidence>(sink)
{
	...
	nCoincPhotopeak = 0;
	nListModeControl = 0;
	for(auto item : systemConfig->CoincPhotopeakTimes)
		nListModeControl += item.size();
}
...
void CoincidenceGrouper::report()
{
	...
	printf("  %10lu \n", nPrompts);
	if(nListModeControl > 0)
		printf("  %10lu of %10lu (%4.1f%%) ListMode validated coincidences are present\n", nCoincPhotopeak, nListModeControl, 100.0*nCoincPhotopeak/nListModeControl);	
